      ...
      setTicker((prev) =>
        prev.map((p) => {
          const newBid = parseFloat(
            (p.bid + (Math.random() - 0.5) * 0.01).toFixed(4)
          );
          const newAsk = parseFloat(
            (p.ask + (Math.random() - 0.5) * 0.01).toFixed(4)
          );
          return {
            ...p,
            bid: newBid,
      ...
      <div className="flex-1 flex flex-col md:ml-64 w-full">
          ...
          <div className="max-w-7xl mx-auto w-full px-3 sm:px-4 lg:px-6 pb-4">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
              {[
                ...
                {
                  value: "$0.00",
                  label: t("Active Deposit"),
                  color: "from-orange-400 to-orange-600",
                },
                {
                  value: "$0.00",
                  label: t("Earned Profit"),
                  color: "from-green-500 to-green-700",
                },
              ].map((card, i) => (
                <Card
                  key={i}
                  className="rounded-2xl shadow-md hover:shadow-lg transition border border-black/5 overflow-hidden"
                >
                  <CardContent
                    className={`bg-gradient-to-br ${card.color} text-white p-4 sm:p-5 lg:p-6 min-h-[150px] flex`}
                  >
                    <div className="flex flex-col justify-between items-start w-full">
                      <div className="space-y-1.5">
                        <div className="text-xl sm:text-2xl font-extrabold leading-none drop-shadow">
                          {card.value}
                        </div>
        ...
        <section className="w-full">
          <div className="max-w-7xl mx-auto w-full px-3 sm:px-4 lg:px-6 py-4 space-y-6">
            {["BTCUSD", "ETHUSD", "XAUUSD"].map((symbol) => (
              <Card key={symbol}>
                <CardContent>
                  <iframe
