...
export default function HelpPage() {
  const [messages, setMessages] = useState([
    { sender: "assistant", text: "👋 Hello! Welcome to VentureWise FX Support. How can we help you today?" },
  ]);
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        { sender: "assistant", text: "✅ Thanks for your message. Our support team will respond shortly." },
      ]);

      // 🔥 stop typing immediately after message is added
      setIsTyping(false);
    }, 2000);
  };
  ...
  return (
    <div className="relative flex flex-col h-[80vh] max-w-2xl mx-auto rounded-2xl overflow-hidden shadow-2xl border border-gray-200">
      ...
      <div className="relative z-10 flex-1 p-4 overflow-y-auto space-y-3 bg-white/70 backdrop-blur-md">
        ...
        {/* Typing indicator (only shows before response is delivered) */}
        {isTyping && (
          <motion.div
          ...
          >
            <div className="px-4 py-2 rounded-2xl bg-gray-200 text-gray-800 text-sm shadow-md">
              <span className="animate-pulse">VentureWise FX is typing...</span>
            </div>
          </motion.div>
        )}
      </div>
